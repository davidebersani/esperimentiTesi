...
curl -L https://raw.githubusercontent.com/kubernetes/ingress-nginx/nginx-0.30.0/deploy/static/provider/baremetal/service-nodeport.yaml | sed 's/targetPort: 80/&\n      nodePort: 31080/' | sed 's/targetPort: 443/&\n      nodePort: 31443/' > /etc/kube-cluster/ingress-nodeport-kube-cluster.yaml
