...
cp ${RESOURCES}/docker.service.d-kube-cluster/hosts.conf /etc/systemd/system/docker.service.d/hosts.conf
...
mkdir -p /etc/docker
cp ${RESOURCES}/etc.docker-kube-cluster/daemon.json /etc/docker/daemon.json
